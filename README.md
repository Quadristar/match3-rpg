...
- 技術: TypeScript(strict)/ Vite / PixiJS v8(WebGL 固定)/ GSAP / Vitest / ESLint
...
**Phase 4b(バトル表示)**。起動すると仮のタイトル画面が表示され、タップするとバトル画面に移ります。

- パネルはドラッグ、または2回タップ(選択 → 隣をタップ)で入れ替えられる
- 揃うと消えて、上から落ちて補充され、連鎖する。連鎖の段数を画面に表示する
- 消えたパネルで敵を攻撃する(ダメージの数字・HP バー)。敵は3ターンごとに攻撃してくる(画面が揺れる)
- 勝敗が決まると「勝利!」「敗北…」と「もう一度」「タイトルへ」ボタンが出る
- 「タイトルへ」ボタンでタイトル画面に戻る
...
`?debug` のとき、バトル画面では各領域(board・enemy・portrait・info)の枠と名前と、盤面のシードを表示します。

`?debug` がなくても使えるパラメータ(ゲーム用):

| パラメータ | 例 | 内容 |
|---|---|---|
| `seed` | `?seed=123` | 盤面の乱数のシードを固定する(同じ操作をすれば同じ結果になる。不具合の再現用)。`?debug&seed=123` と併用できる |
